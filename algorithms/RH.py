...
def right_hand(maze):
    ...
    goal = maze.goal
    current = start
    direction = "right"  # Starting facing right (consistent naming)
    ...
    while current != goal and iteration_count < max_iterations:
        ...
        # Right-hand rule: try right, forward, left, back (in that order)
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) != 2:
        sys.exit("Usage: python RH.py maze.txt")  # Fixed filename reference
